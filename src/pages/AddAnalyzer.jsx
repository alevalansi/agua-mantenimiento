...
export default function AddAnalyzer() {
  ...
  const { stations, refresh, logActivity, currentTechnician } = useApp();
  ...
  const handleSubmit = (e) => {
    ...
    const list = getList(KEYS.ANALYZERS);
    const newAnalyzer = {
    ...
    list.push(newAnalyzer);
    setList(KEYS.ANALYZERS, list);
    refresh(KEYS.ANALYZERS);

    logActivity({
      technician_name: currentTechnician?.name,
      action_type: 'add_analyzer',
      description: `הוספת מנתח ${newAnalyzer.name} לתחנה ${station?.name || ''}`,
      station_name: station?.name,
      analyzer_name: newAnalyzer.name,
    });
        ...
        <Card className="p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              <Button type="submit" disabled={!form.name.trim() || (!form.station_id)} className="flex-1 justify-center">
                הוסף מנתח
              </Button>
